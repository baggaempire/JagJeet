...
struct JapjiPacketView: View {
    ...
    private var entries: [ReflectionEntry] { appViewModel.japjiEntries }
    ...
    private var unfinishedIndices: [Int] {
        entries.indices.filter { !appViewModel.isJapjiCompleted(entries[$0]) }
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 14) {
                ...
                if let entry = currentEntry {
                    ...
                    Text(language.text(.noJapjiAvailable))
        ...
        .navigationTitle("Japji Sahib \(language.text(.pathLabel))")
        ...
        .alert(language.text(.pathComplete), isPresented: $showRestartPrompt) {
            ...
            Text(language.text(.japjiRestartMessage))
        }
    }

    private var header: some View {
        VStack(spacing: 6) {
            Text("\(appViewModel.japjiCompletedCount) / \(appViewModel.japjiTotalCount) \(language.text(.ofCompleted))")
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                HStack {
                    ...
                    if appViewModel.isJapjiCompleted(entry) {
                ...
                JapjiReflectionSection(title: language.text(.gurmukhiVerse), content: entry.gurmukhiText, isGurmukhi: true)
                JapjiReflectionSection(title: reflectionMeaningLabel(for: language), content: entry.englishMeaning)
                JapjiReflectionSection(title: language.text(.simpleExplanation), content: entry.simpleExplanation)
                JapjiReflectionSection(title: language.text(.reflectionForToday), content: entry.lifeReflection)
            }
            .padding(24)
        }
        .frame(maxHeight: 560)
    ...
    private func markCompleteAndAdvance() {
        ...
        appViewModel.markJapjiCompleted(entry)
    ...
    private func runCompletionMomentAndAdvance() async {
        ...
        if let nextIndex = nextUnfinishedIndex(after: currentIndex) ?? unfinishedIndices.first {
            ...
            appViewModel.updateJapjiCurrentIndex(currentIndex)
            transitionOffset = 28
            cardOpacity = 0
    ...
    private func prepareStartingIndex() {
        ...
        let savedIndex = min(appViewModel.japjiPacketProgress.currentIndex, max(entries.count - 1, 0))
        if entries.indices.contains(savedIndex), !appViewModel.isJapjiCompleted(entries[savedIndex]) {
    ...
    private func nextUnfinishedIndex(after index: Int) -> Int? {
        ...
        if start < entries.count {
            for candidate in start..<entries.count where !appViewModel.isJapjiCompleted(entries[candidate]) {
                return candidate
            }
        }
        for candidate in 0..<min(index + 1, entries.count) where !appViewModel.isJapjiCompleted(entries[candidate]) {
            return candidate
        }
        return nil
    }
    ...
    private func previousUnfinishedIndex(before index: Int) -> Int? {
        ...
        while candidate >= 0 {
            if !appViewModel.isJapjiCompleted(entries[candidate]) {
                return candidate
            }
            candidate -= 1
        }
    ...
    private func restartPacket() {
        appViewModel.resetJapjiProgress()
        currentIndex = 0
        appViewModel.updateJapjiCurrentIndex(currentIndex)
    }
}

private struct JapjiReflectionSection: View {
    let title: String
    let content: String
